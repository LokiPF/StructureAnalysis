...
def calc_buckling_factors(b: float, a: float, t: float, mu: float, E: float, sigma_y: float, sigma_x: float) -> [float,
                                                                                                                 float]:
    """Function to minimize the stress and calculate the number of halve waves in x- and y-direction"""
    alpha = a / b
    beta = sigma_y / sigma_x
    min_m, min_n, min_sigma = 1, 1, -1
    k_biax = 0
    for m in range(10):
        for n in range(10):
            k_sigma = np.square(np.square(m) + np.square(n * alpha)) / (
                        np.square(alpha) * (np.square(m) + beta * np.square(n * alpha)))
            sigma_e = np.square(E * np.pi) / (12 * np.square(1 - mu)) * np.square(t / b)
            sigma_crit = k_sigma * sigma_e
            if sigma_crit < min_sigma or min_sigma == -1:
...
def calc_avg_sigma_panel(load_cases: [LoadCase], params: Parameters) -> Panel:
    for load_case in load_cases:
        for i in range(5):
            ...
            buckling_f = calc_buckling_factors(400, 600, 4, 0.34, 64744.31, avg_sigma[1], avg_sigma[0])
...
def calc_avg_sigma_combined(load_cases: [LoadCase], E):
    for load_case in load_cases:
        for i in range(3):
            ...
            avg_sigma_panel = avg_panel(panel) * 6 * 160000
            sigma_combined = (avg_stringer_xx + avg_sigma_panel[0]) / (56800 * 3 + 160000 * 6)
